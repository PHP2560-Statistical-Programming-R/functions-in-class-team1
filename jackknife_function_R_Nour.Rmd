...
mean.jackknife <- function(a_vector) {
  for (i in seq_along(a_vector))
    a_vector_mean[i] <- mean(a_vector[-i]) 
      jackknife<-  var(a_vector_mean) * (((length(a_vector) - 1)^2) / length(a_vector))
    jackknife.stderr <- sqrt(jackknife)
...
jackknife.lm <- function(df,formula,p) {
  n = nrow(df)
  coeff.mat <- matrix(nrow = n, ncol = p)
  jackknife.stderr = vector(length = p)
  for(i in 1:n) {
    dflm <- lm(formula = formula , data = df[-i,] )
   coeff.mat[i,] <- coefficients(dflm)
    var = apply(coeff.mat, 2, var)
