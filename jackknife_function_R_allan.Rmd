...
mean.jackknife <- function(a_vector) {
  ...
  n <- length(a_vector)
  estimitors <- vector(length = n)
  for (i in 1:n) {
  estimitors[i] = mean(a_vector[-i])
  }
  var.estimators = var(estimitors)
  jackknife.var <- ((n-1)^2/n)* var.estimators 
...
gamma.jackknife <- function(a_vector) {
  n <- length(a_vector)
  estimators <- matrix(NA,nrow=2,ncol=n)
  rownames(estimators) = c("a","s")
  for (i in 1:n) {
    est <- gamma.est(a_vector[-i])
    estimators["a",i] <- est["a"]
    estimators["s",i] <- est["s"]
  }
  var.estimators <- apply(estimators,1,var)
  jackknife.vars <- ((n-1)^2/n)*var.estimators
...
jackknife.lm <- function(df,formula,p) {
  nrows <- nrow(df)
  estimators <- matrix(0,nrow=p,ncol=n)
  for (omit in 1:nrows) {
    new.coefs <- lm(as.formula(formula),data=df[-omit,])$coefficients
    estimators[,omit] <- new.coefs
  }
  var.estimators <- apply(estimators,1,var)
  jackknife.var <- ((nrow-1)^2/nrow)*var.estimators
  jackknife.stderr <- sqrt(jackknife.var)
  return(jackknife.stderr)
}
```
...
- Write a function `omit.case` that omits a point given the data and returns the data minus that point. Make sure it can handle higher dimensions. 
    ...
    omit.case <- function(the_data,omitted_point) {
      ...
      new_data <- NULL
      if (length(dim(the_data))==1) {
        new_data <- (the_data[-omitted_point])
      } else if(is.null(dim(the_data))){
...
jackknife <- function(estimator,the_data) {
  ...
  omit_and_est <- function(omit) {
    ...
    omt <- omit.case(data, omit)
    est <- estimator(omt)
    return(est)
  }
  jackknife.ests <- matrix(sapply(1:n, 
  omit_and_est), ncol = n)
  estimates2 <- apply(jackknife.ests, 1, var)
  var <- ((n - 1) ^ 2 / n) * estimates2
  return(sqrt(var))
}
```
...
scale_and_sqrt_vars <- function(jackknife.ests,n) {
  estimates2 <- apply(jackknife.ests,1,var)
  jackknife.stderr <- sqrt(((n-1)^2/n)* estimates2)
  return(jackknife.stderr)
}
```
