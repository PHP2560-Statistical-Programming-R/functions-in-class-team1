...
gamma.jackknife <- function(a_vector) {
  ...
  s_s = vector(length = n)
  for(i in 1:n){
    a_s[i] = gamma.est(a_vector[-i])["a"]
    s_s[i] = gamma.est(a_vector[-i])["s"]
  }
  jackknife.stderrs = vector(length = 2)
  jackknife.stderrs[1] = mean.jackknife(a_s)
  jackknife.stderrs[2] = mean.jackknife(s_s)
...
jackknife.lm <- function(df,formula,p) {
  ...
  for(i in 1:p){
    jackknife.stderr[i] = mean.jackknife(coef_list[,i])
  }
  return(jackknife.stderr)
}
```
